    ...
    def get_evaluation(self, sess, batch):
        ...
        if self.config.mode == 'server':
          global_step, yp, yp2, loss, vals, na, u, h, p0, g1, g2 = sess.run([
              ...
              list(self.tensor_dict.values()), self.na, self.u, self.h, self.p0, 
              self.g1, self.g2], feed_dict=feed_dict)
          u_l, h_l, p0_l, g1_l, g2_l = [x.tolist() for x in (u, h, p0, g1, g2)]
        else:
          global_step, yp, yp2, loss, vals, na  = sess.run([
              self.global_step, self.yp, self.yp2, self.loss, 
              list(self.tensor_dict.values()), self.na], feed_dict=feed_dict)
        ...
        id2answer_dict['na'] = id2na_dict
        tensor_dict = dict(zip(self.tensor_dict.keys(), vals))
        e = ForwardEvaluation(data_set.data_type, int(global_step), idxs, 
                              yp.tolist(), yp2.tolist(), float(loss), 
                              id2answer_dict, na.tolist(), u_l, h_l, p0_l, 
                              g1_l, g2_l, tensor_dict=tensor_dict)
        return e
